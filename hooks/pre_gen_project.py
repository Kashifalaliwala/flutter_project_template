...
if re.search(PACKAGE_REGEX, package_name):
    ...
    print(
        CRED + 'Avoid using any special characters. Only alphanumeric characters are allowed.' + CEND)
...
print(
    CGREEN + 'Proceeding with app name: {}, package name: {}'.format(app_name, package_name) + CEND)
...
if splash_image_path != 'Required':
    ...
    if not os.path.exists(splash_image_path):
        print(CRED + '\n Splash image does not exists \n' + CEND)
        sys.exit(1)
    elif not (fileType == '.jpeg' or fileType == '.jpg' or fileType == '.png'):
